...
#![allow(missing_docs)]
use core::{
    marker::PhantomData, mem::{self, ManuallyDrop, MaybeUninit}, ptr::{addr_of_mut, NonNull}
};
...
// use crate::iio::channels;
// use crate::iio::trigger;
// pub mod buffer;
...
impl<T: Driver> Device<T> {
        ...
        P: PinInit<T::Data, Error>,
    {
        ...
        unsafe {
            ...
            addr_of_mut!((*indio_dev.as_ptr()).num_channels)
...
pub trait Driver: Sized {
    ...
    fn read_raw(device: &Device<Self>, spec: &Specification) -> Result<SensorData> {
        build_error!(VTABLE_DEFAULT_ERROR)
    }

    fn write_raw(device: &Device<Self>, spec: &Specification, sdata: SensorData) -> Result {
        build_error!(VTABLE_DEFAULT_ERROR)
    }
}
...
impl<T: Driver> IioVTableAdapter<T> {
    unsafe extern "C" fn read_raw(
        ...
        let device: ManuallyDrop<Device<T>> = ManuallyDrop::new(Device::from_raw(indio_dev));
